                               ...
                               const misc_params * m_pars )
{
   ...
   #pragma simd 
   for (atomi=0; atomi < myatoms->N; atomi++) //currently will not vectorize
   {
   ...
   float sum_local_pot_array[myatoms->N];
   float sum_local_virial_array[myatoms->N];
   memset(sum_local_pot_array, 0, myatoms->N * sizeof(sum_local_pot_array[0]));
   memset(sum_local_virial_array, 0, myatoms->N * sizeof(sum_local_virial_array[0]));


   for (atomi=0; atomi < myatoms->N; atomi++)
   {
     float sum_local_pot_TOTAL = 0;
     float sum_local_virial_TOTAL = 0;
	     ...
	     if (atomj != atomi)
	       {
		    ...
		    float pot_energy_local = 0;
    		
    		    pot_energy_local = len_jo->sig12 * dis12i - len_jo->sig6 * dis6i; 
    		    sum_local_pot_array[atomj] += pot_energy_local;
    		
            float fterm = dis2i * ( 2.0 * len_jo->sig12 * dis12i -
                                              len_jo->sig6 * dis6i );
    		
            float virial_local = 0;
            virial_local = fterm * dis2; 
    	    sum_local_virial_array[atomj]-=virial_local;
   ...
   for (atomi=0; atomi < myatoms->N; atomi++)
   {
      myatoms->pot_energy += sum_local_pot_array[atomi];
      myatoms->virial += sum_local_virial_array[atomi];
